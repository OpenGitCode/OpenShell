...
1. Download the runtime for your OS (extracts into `$OPENSHELL_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.openshell/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$OPENSHELL_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.openshell/tools/sherpa-onnx-tts/models`)

Resolve the active state directory first:

```bash
STATE_DIR="${OPENSHELL_STATE_DIR:-$HOME/.openshell}"
```

Then write those resolved paths into the active OpenShell config file (`$OPENSHELL_CONFIG_PATH`, default `~/.openshell/openshell.json`):
