---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenShell deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenShell cron scheduling for periodic checks, or version status checks on a machine running OpenShell (laptop, workstation, Pi, VPS).
---

# OpenShell Host Hardening

## Overview

Assess and harden the host running OpenShell, then align it to a user-defined risk tolerance without breaking access. Use OpenShell security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenShell changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenShell gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenShell can access sensitive data.
...
### 2) Run OpenShell security audits (read-only)

As part of the default read-only checks, run `openshell security audit --deep`. Only offer alternatives if the user requests them:

1. `openshell security audit` (faster, non-probing)
2. `openshell security audit --json` (structured output)

Offer to apply OpenShell safe defaults (numbered):

1. `openshell security audit --fix`

Be explicit that `--fix` only tightens OpenShell defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenShell version/update status (read-only)

As part of the default read-only checks, run `openshell update status`.
...
- Credential hygiene notes (location of OpenShell creds, prefer disk encryption)
...
- OpenShell security audit (re-run)
...
After OpenShell install or first hardening pass, run at least one baseline audit and version check:

- `openshell security audit`
- `openshell security audit --deep`
- `openshell update status`

Ongoing monitoring is recommended. Use the OpenShell cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `openshell cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `openshell update status`
...
Before creating, `openshell cron list` and match on exact `name`. If found, `openshell cron edit <id> ...`.
If not found, `openshell cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `openshell update status` (preferred for source checkouts and channels)
2. `npm view openshell version` (published npm version)

## OpenShell command accuracy

Use only supported commands and flags:

- `openshell security audit [--deep] [--fix] [--json]`
- `openshell status` / `openshell status --deep`
- `openshell health --json`
- `openshell update status`
- `openshell cron add|list|runs|run`

Do not invent CLI flags or imply OpenShell enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenShell compaction:
